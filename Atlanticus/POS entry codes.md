

- ﻿﻿90 - Full Track 1 and 2 (Card swiped without using the chij
- ﻿﻿91 - Contactless magnetic stripe (mobile wallet)
- ﻿﻿95- Initiated by chip card: CVV data may be unreliable
- ﻿﻿00 - Pan Entry Unknown
- ﻿﻿01 - Manually Keyed
- ﻿﻿02 - Magnetic Stripe Read
- ﻿﻿05 - Chip Read
- ﻿﻿04-OCR Read
- ﻿﻿06 - Track 1 read
- ﻿﻿07 - Contactless chip
- ﻿﻿10 - Credential on file
- ﻿﻿79 - Chip and magnetic stripe read failed, fallback to magnetic stripe
- ﻿﻿80 - Chip card or chip- capable terminal was unable to process transaction using data on the chip
- ﻿﻿81 - E-commerce and internet

a 1-4 of 15

P

Q Search